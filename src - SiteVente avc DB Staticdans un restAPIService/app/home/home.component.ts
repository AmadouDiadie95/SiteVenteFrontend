...
export class HomeComponent implements OnInit {
  ...
  allProducts: Array<ProductModel> = this.restApiService.allProducts ;
  allCategories: Array<CategoryModel> = this.restApiService.allCategories ;
  allCommandes: Array<CommandeModel> = this.restApiService.allCommandes ;
  ...
  ngOnInit(): void {
    ...
    this.initAllImage2() ;
    this.initAllProducts2() ;

  }

  initAllImage2(){
    this.allImages.forEach(image => {
      if (image.identifier == 'header-caroussel-1') {
        this.imageHeaderCar1 = image ;
      }
      if (image.identifier == 'header-caroussel-2') {
        this.imageHeaderCar2 = image ;
      }
      if (image.identifier == 'header-banner-1') {
        this.imageHeaderBanner1 = image ;
      }
      if (image.identifier == 'header-banner-2') {
        this.imageHeaderBanner2 = image ;
      }
      if (image.identifier == 'header-banner-3') {
        this.imageHeaderBanner3 = image ;
      }
      if (image.identifier == 'middle-banner-1') {
        this.imageMiddleBanner = image ;
      }
      if (image.identifier == 'middle-banner-2') {
        this.imageMiddleBanner2 = image ;
      }
    }) ;
  }
  ...
  initAllProducts2(){
    this.allProducts.forEach(product => {
      if (product.populary) {
        this.allPopularsProducts.push(product) ;
      }
      if (product.newProduct) {
        this.allNewProducts.push(product) ;
      }
      if (product.particularity == 'promoProduct1') {
        this.promoProduct1 = product ;
      }
      if (product.particularity == 'promoProduct2') {
        this.promoProduct2 = product ;
      }
    }) ;
  }
